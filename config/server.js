...
export const createServer = (db) => {
	...
	app.post("/scrape", async (req, res) => {
		...
		try {
			...
			const inDb = await db.updateOne({}, { $set: { data: eventDetails, updatedAt: new Date() } }, { upsert: true });
			console.log(inDb);
	...
	app.get("/", async (req, res) => {
		try {
			...
			return data ? res.status(200).send(data) : res.status(404).send({ error: true, message: "Data not found" });
		} catch (err) {
