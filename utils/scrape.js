...
const scrape = async () => {
    ...
    for (const event of events) {
        ...
        const fights = $event('li.fightCard:not(.picks)').map((_, el) => {
            const main = $event(el).find('.billing').text().toLowerCase().includes('main') ? true : false;
            const weight = $event(el).find('.fightCardWeight .weight').text().trim();
            
            const fighterA = {
                name: $event(el).find('.fightCardFighterName.left a').text().trim(),
                record: $event(el).find('.fightCardFighterBout.left .fightCardRecord').text().trim(),
                country: $event(el).find('.fightCardFighterBout.left .fightCardFlag').attr('src'),
                picture: $event(el).find('.fightCardFighterImage img').first().attr('src'),
                link: baseUrl + $event(el).find('.fightCardFighterBout.left a').attr('href'),
            };

            const fighterB = {
                name: $event(el).find('.fightCardFighterName.right a').text().trim(),
                record: $event(el).find('.fightCardFighterBout.right .fightCardRecord').text().trim(),
                country: $event(el).find('.fightCardFighterBout.right .fightCardFlag').attr('src'),
                picture: $event(el).find('.fightCardFighterImage img').last().attr('src'),
                link: baseUrl + $event(el).find('.fightCardFighterBout.right a').attr('href'),
            };
